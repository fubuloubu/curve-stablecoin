...
class StatefulLendBorrow(RuleBasedStateMachine):
    ...
    rate = st.integers(
        min_value=0, max_value=2**255 - 1
    )  # Negative is probably not good btw
    ...
    def create_loan(self, c_amount, amount, n, user_id):
        ...
        with boa.env.prank(user):
            ...
            if self.controller.total_debt() + amount > self.debt_ceiling:
                if (
                    (self.controller.total_debt() + amount) * self.amm.get_rate_mul()
                    > 2** 256 - 1
    ...
    def add_collateral(self, c_amount, user_id):
        ...
        with boa.env.prank(user):
            ...
            try:
                ...
                if (c_amount + self.amm.get_sum_xy(user)[1]) < (
                    2**128 - 1
                ) // DEAD_SHARES:
                    raise
    ...
    def borrow_more(self, c_amount, amount, user_id):
        ...
        with boa.env.prank(user):
            ...
            if self.controller.total_debt() + amount > self.debt_ceiling:
                if (
                    self.controller.total_debt() + amount
                ) * self.amm.get_rate_mul() > 2**256 - 1:
                    with boa.reverts():
                        self.controller.borrow_more(c_amount, amount)
                else:
                    with boa.reverts():
    ...
    def sum_of_debts(self):
        assert abs(
            sum(self.controller.debt(u) for u in self.accounts)
            - self.controller.total_debt()
        ) <= len(self.accounts)
...
def test_too_deep(
    ...
    with boa.env.anchor():
        ...
        state.create_loan(
            amount=10161325728155098164, c_amount=4156800770, n=50, user_id=2
        )
...
def test_overflow(
    ...
    state.sum_of_debts()
    state.borrow_more(
        amount=102598604287098624639570500830661495567,
        c_amount=12171265979771193868,
        user_id=0,
    )
    state.debt_payable()
    state.sum_of_debts()
