...
def test_leverage(
    ...
    with boa.env.prank(user):
        ...
        market_controller.repay(
            0, user, market_amm.active_band(), fake_leverage.address, calldata
        )
        fake_leverage_balances3 = [
            stablecoin.balanceOf(fake_leverage),
            collateral_token.balanceOf(fake_leverage),
        ]
    ...
    with boa.env.prank(user):
        ...
        if (
            ...
            market_controller.borrow_more(
                amount, more_debt, user, fake_leverage.address, calldata
            )
            debt += more_debt
            if more_debt > 0:
                assert collateral_token.balanceOf(user) == 0
                ...
                assert xy[0] == 0
                assert xy[1] == pytest.approx(expected_collateral, rel=1e-9, abs=10)
                assert market_controller.debt(user) == pytest.approx(
                    debt, rel=1e-9, abs=10
                )
                assert stablecoin.balanceOf(user) == 0
        ...
        if debt * int(repay_mul * 1e18) // 10**18 >= 1:
            ...
            return
        assert (
            market_controller.debt(user)
            == debt - debt * int(repay_mul * 1e18) // 10**18
        )
        if repay_mul == 1.0:
            assert collateral_token.balanceOf(market_amm.address) == 0
            assert stablecoin.balanceOf(market_amm.address) == 0
        ...
        assert collateral_token.balanceOf(market_controller.address) == 0
        assert (
            stablecoin.balanceOf(market_controller.address) - s0
            == debt * int(repay_mul * 1e18) // 10**18
        )
        if repay_mul == 1.0:
            if more_debt > 0:
